...
| `feet_SG90.step` / `.stl` | base interface (stock, no mounting holes) |
| `feet_SG90M.*`, `feet_MG90S2M.*` | **screw-down feet variants** for `../enclosure/pedestal-sg90` — generated by `make_feet_mounting.py` (see "Feet M variants" below) |
...
## Feet "M" variants — screw-down mounting for the pedestal (2026-06-12)

`make_feet_mounting.py` generates `feet_SG90M` and `feet_MG90S2M` (STEP + STL):
the stock feet with **4× Ø1.6 × 6.5 mm pilot holes** drilled up from the bottom
face at feet-local **(±12.5, −18)** and **(±12.5, +10)** (hub = origin). Print
these instead of the stock feet when using `../enclosure/pedestal-sg90`.

Why: the pedestal recess was sized 54×50 from the *Takao* feet, but the vendored
v1.0 `feet_SG90` actually measures **48 × 37 × 8.2 mm** (rounded-rect r2.0; bbox
center 4.5 mm off the hub; 2 mm-wide full-height cable slot on one edge; horn
pocket floor at Z 5.7). The reworked pedestal (same 2026-06-12 change) has a
48.4 × 37.4 recess with a 2 mm floor, a back wire slot, and 4 matching Ø2.4
clearance holes — the feet drop in (cable slot toward the barrel jack; the
offset recess keys the orientation) and are retained by **4× M2×8 pan-heads
driven from inside the cavity**. Hole spots were ray-cast verified solid through
the full 8.2 mm height; 6.5 mm pilots clear the horn pocket (which the pattern
avoids entirely). The pan-horn M2×8 stays serviceable through the pedestal's
Ø22 center hole.
